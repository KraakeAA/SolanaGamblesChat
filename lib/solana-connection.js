...
// --- VERSION: 2.3.0-reverted-to-hardcoded-keys ---
...
const QUICKNODE_WSS_URL = 'wss://multi-young-tab.solana-mainnet.quiknode.pro/56662595a48eb3798b005654091f77aa5673e15e/'; // Manually converted
...
const HARCODED_RPC_ENDPOINTS = [
    ...
    // Example of a public fallback (less reliable, no API key)
    // { 
...
class RateLimitedConnection extends Connection {
    constructor(endpointsFromEnv = [], options = {}) { 
            ...
            envUrls.forEach(url => {
                if (url.startsWith('http')) { 
                    const matchingHardcoded = HARCODED_RPC_ENDPOINTS.find(hardcoded => 
                        url.startsWith(hardcoded.http.split('?')[0]) // Compare base URL part
                    );
        ...
        if (effectiveRpcEndpoints.length === 0) {
            console.error("[RateLimitedConnection] CRITICAL: No RPC endpoints available after all checks (hardcoded list might be empty or env vars misconfigured). RPC calls will fail.");
        ...
        this.initialWsEndpointUserProvided = !!options.wsEndpoint;
        let primaryWsEndpoint = options.wsEndpoint || effectiveRpcEndpoints[0].ws;
