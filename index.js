...
async function handleStartCoinflipUnifiedOfferCommand(msg, betAmountLamports, targetUsernameRaw = null) {
    ...
    const logPrefix = `[CF_OfferOrDirect_UserCap_CanonLock_V4 UID:${userId} CH:${chatId}]`; // Updated log

    // --- Global User Game Cap Check ---
    ...
    if (!initiatorUserObj) { /* ... error handling ... */ return; }
    ...
    if (BigInt(initiatorUserObj.balance) < betAmountLamports) { /* ... insufficient balance ... */ return; }
    ...
    if (targetUsernameRaw) {
        ...
        if (!targetUserObject || !targetUserObject.telegram_id) {
            await safeSendMessage(chatId, `😕 Player ${escapeHTML(targetUsernameRaw)} not found. To make a general Coinflip offer, use <code>/cf ${escapeHTML(args[0] || "<bet>")}</code>.`, { parse_mode: 'HTML' });
            return;
        } else if (String(targetUserObject.telegram_id) === userId) {
            await safeSendMessage(chatId, `😅 You can't challenge yourself to Coinflip, ${initiatorPlayerRefHTML}! To make a general offer, use <code>/cf ${escapeHTML(args[0] || "<bet>")}</code>.`, { parse_mode: 'HTML' });
            return;
        }
    }

    // --- Per-Group, Per-Game-Family Lock Check for Coinflip ---
    ...
    if (gameSession.activeGamesByTypeInGroup.has(COINFLIP_FAMILY_LOCK_KEY)) {
        const existingGameIdInGroup = gameSession.activeGamesByTypeInGroup.get(COINFLIP_FAMILY_LOCK_KEY);
        const existingGameInstance = activeGames.get(existingGameIdInGroup);
        const nonTerminalStates = [ /* ... comprehensive list ... */
            'pending_offer', 'pending_unified_offer', 'pending_direct_challenge_response',
            'awaiting_difficulty', 'player_turn_awaiting_emoji', 'player_score_18_plus_awaiting_choice',
            'p1_awaiting_roll1_emoji', 'p2_awaiting_roll1_emoji', 'p1_awaiting_roll_emoji', 'p2_awaiting_roll_emoji',
            'bot_rolling_internal', 'initiator_turn', 'opponent_turn', 'player_initial_roll_1_prompted',
            'player_initial_roll_2_prompted', 'player_turn_hit_stand_prompt', 'dealing_initial_hands',
            'pvp_p1_choosing', 'pvp_p2_choosing', 'pvb_waiting_player_choice', 'player_action_processing_stand',
            'bot_turn_pending_rolls', 'bot_rolling', 'awaiting_single_roll', 'processing_roll',
            'spinning_waiting_helper', 'player_turn', 'bot_turn', 'waiting_for_choice',
            'awaiting_player_choice', 'pvp_p1_chosen_wait_p2', 'pvp_p2_chosen_wait_p1' // CF/RPS states
        ];
        if (existingGameInstance && (nonTerminalStates.includes(existingGameInstance.status) || (existingGameInstance.status && !existingGameInstance.status.startsWith('game_over_')))) {
            await safeSendMessage(chatId, `⏳ Hold your coins, ${initiatorPlayerRefHTML}! A Coinflip game or offer is already active in this group. Please wait.`, { parse_mode: 'HTML' });
            return;
        } else if (!existingGameInstance) {
            console.warn(`${logPrefix} Stale Coinflip family lock (${COINFLIP_FAMILY_LOCK_KEY}) found. Clearing.`);
    ...
    if (targetUserObject && targetUserObject.telegram_id) {
        // --- DIRECT PvP CHALLENGE OFFER for Coinflip ---
        ...
        if (!sentGroupMessage || !sentGroupMessage.message_id) { /* ... error handling ... */ return; }
        // ... (DM notification) ...
        ...
        offerData = {
            ...
            gameToStart: GAME_IDS.COINFLIP_PVP, creationTime: Date.now()
        };
    } else {
        // --- UNIFIED Coinflip OFFER ---
        ...
        const offerMessageTextHTML = `👑 ${COIN_EMOJI_DISPLAY} <b>A Coinflip Challenge Has Been Issued!</b> ${COIN_EMOJI_DISPLAY} 👑\n\n` +
            ...
            `<i>This offer expires in ${JOIN_GAME_TIMEOUT_MS / 1000 / 60} minutes!</i>`;
        const offerKeyboard = { /* ... */
            inline_keyboard: [
        ...
        if (!sentMessage?.message_id) { /* ... error handling ... */ return; }

        offerData = {
            type: GAME_IDS.COINFLIP_UNIFIED_OFFER, gameId: offerId, chatId: chatId, chatType: chatType,
            initiatorId: userId, initiatorMentionHTML: initiatorPlayerRefHTML, initiatorUserObj: initiatorUserObj,
            betAmount: betAmountLamports, status: 'pending_offer', // Or 'awaiting_player_choice'
            creationTime: Date.now(), offerMessageId: String(sentMessage.message_id)
        };
    }

    activeGames.set(offerId, offerData);
    await updateGroupGameDetails(chatId, offerId, COINFLIP_FAMILY_LOCK_KEY, betAmountLamports);
    console.log(`${logPrefix} Offer ${offerId} (Actual Type: ${offerData.type}) created. Group lock set for Coinflip family (${COINFLIP_FAMILY_LOCK_KEY}) with ID: ${offerId}.`);

    setTimeout(async () => { // Common timeout logic for the offer
        const timedOutOffer = activeGames.get(offerId);
        if (timedOutOffer &&
            ((timedOutOffer.type === GAME_IDS.DIRECT_PVP_CHALLENGE && timedOutOffer.status === 'pending_direct_challenge_response') ||
             (timedOutOffer.type === GAME_IDS.COINFLIP_UNIFIED_OFFER && (timedOutOffer.status === 'pending_offer' || timedOutOffer.status === 'awaiting_player_choice')))) {
            activeGames.delete(offerId);
            await updateGroupGameDetails(chatId, null, COINFLIP_FAMILY_LOCK_KEY, null);
            // ... (message editing logic for timeout) ...
        }
...
    initiatorData, // Object containing initiator's details and game context OR just initiatorUserObj if betsAlreadyDeducted
...
    betsAlreadyDeducted = false // Flag indicating if bets were handled by the caller
) {
    // Determine chatId from initiatorData, which might be a full offer object or just user details
    const chatId = String(initiatorData.chatId || (initiatorData.initiatorUserObj ? initiatorData.initiatorUserObj.chat?.id : null) || originalChatId); // Fallback needed if initiatorData is just userObj
    // If chatId is still null, this is an issue. A more robust way to get chatId might be needed if initiatorData structure varies a lot.
    // For now, assuming initiatorData.chatId is the primary source from unified offers.

    const logPrefix = `[CF_PvP_Start_NoDoubleBet_FamilyLockFix Init:${initiatorData.initiatorUserObj?.telegram_id || initiatorData.initiatorId} Join:${joinerUserObjFull.id || joinerUserObjFull.telegram_id} Chat:${chatId}]`;

    const betAmount = BigInt(initiatorData.betAmount);
    
    let initiatorUserObj = initiatorData.initiatorUserObj || initiatorData; // Handle if initiatorData is just the user object
...
    }
    
    // If this PvP game started from a unified offer, that offer object (initiatorData) should be deleted from activeGames
    // If initiatorData.gameId is the offer's ID
    if (initiatorData.gameId && (initiatorData.type === GAME_IDS.COINFLIP_UNIFIED_OFFER || initiatorData.type === GAME_IDS.DIRECT_PVP_CHALLENGE) && activeGames.has(initiatorData.gameId)) {
        activeGames.delete(initiatorData.gameId); 
        console.log(`${logPrefix} Deleted original offer ${initiatorData.gameId} as Coinflip PvP game starts.`);
    }

    let client;
    const pvpGameId = generateGameId(GAME_IDS.COINFLIP_PVP);
    const COINFLIP_FAMILY_LOCK_KEY = GAME_IDS.COINFLIP_UNIFIED_OFFER; // Define for clarity
...
            client = await pool.connect(); await client.query('BEGIN');
            
            // Re-fetch user objects for balance to ensure atomicity if not already fresh
            const currentInitiatorForBetDeduction = await getOrCreateUser(initiatorId, null, null, null, client); 
            const currentJoinerForBetDeduction = await getOrCreateUser(joinerId, null, null, null, client);

            if (!currentInitiatorForBetDeduction || BigInt(currentInitiatorForBetDeduction.balance) < betAmount) {
                 throw new Error(`Initiator ${initiatorMentionHTML} has insufficient funds (${currentInitiatorForBetDeduction ? currentInitiatorForBetDeduction.balance : 'N/A'}) for Coinflip PvP bet of ${betAmount}.`);
            }
            if (!currentJoinerForBetDeduction || BigInt(currentJoinerForBetDeduction.balance) < betAmount) { 
                 throw new Error(`Joiner ${joinerMentionHTML} has insufficient funds (${currentJoinerForBetDeduction ? currentJoinerForBetDeduction.balance : 'N/A'}) for Coinflip PvP bet of ${betAmount}.`);
            }
...
            initiatorUserObj.balance = initBetRes.newBalanceLamports; // Update the object passed in or fetched locally
...
            joinerUserObjFull.balance = joinBetRes.newBalanceLamports; // Update the object passed in
            
            await client.query('COMMIT');
        } catch (error) {
            if (client) await client.query('ROLLBACK').catch(() => {});
...
    activeGames.set(pvpGameId, gameDataPvP);
    // *** MODIFIED LINE FOR CANONICAL FAMILY ID LOCK ***
    await updateGroupGameDetails(chatId, pvpGameId, COINFLIP_FAMILY_LOCK_KEY, betAmount); 
    // *** END OF MODIFICATION ***
...
async function handleStartRPSUnifiedOfferCommand(msg, betAmountLamports, targetUsernameRaw = null) {
    ...
    const chatId = String(msg.chat.id);
    const chatType = msg.chat.type;
    const logPrefix = `[RPS_OfferOrDirect_UserCap_CanonLock_V4 UID:${userId} CH:${chatId}]`; // Updated log

    // --- Global User Game Cap Check ---
    const activeUserGameCheck = await checkUserActiveGameLimit(userId);
    if (activeUserGameCheck.limitReached) { /* ... message ... */ return; }

    if (chatType === 'private') { /* ... message ... */ return; }
    ...
    let targetUserObject = null;
    if (targetUsernameRaw) { /* ... target user parsing, same as Coinflip ... */
        targetUserObject = await findRecipientUser(targetUsernameRaw);
        if (!targetUserObject || !targetUserObject.telegram_id) {
            await safeSendMessage(chatId, `😕 Player ${escapeHTML(targetUsernameRaw)} not found. To make a general RPS offer, use <code>/rps ${escapeHTML(args[0] || "<bet>")}</code>.`, { parse_mode: 'HTML' });
            return;
        } else if (String(targetUserObject.telegram_id) === userId) {
            await safeSendMessage(chatId, `😅 You can't challenge yourself to RPS, ${initiatorPlayerRefHTML}! To make a general offer, use <code>/rps ${escapeHTML(args[0] || "<bet>")}</code>.`, { parse_mode: 'HTML' });
            return;
        }
    }

    // --- Per-Group, Per-Game-Family Lock Check for RPS ---
    const gameSession = await getGroupSession(chatId, msg.chat.title || `Group Chat ${chatId}`);
    const RPS_FAMILY_LOCK_KEY = GAME_IDS.RPS_UNIFIED_OFFER;

    if (gameSession.activeGamesByTypeInGroup.has(RPS_FAMILY_LOCK_KEY)) {
        const existingGameIdInGroup = gameSession.activeGamesByTypeInGroup.get(RPS_FAMILY_LOCK_KEY);
        const existingGameInstance = activeGames.get(existingGameIdInGroup);
        const nonTerminalStates = [ /* ... comprehensive list from Coinflip, including RPS specific states ... */
            'pending_offer', 'pending_unified_offer', 'pending_direct_challenge_response',
            'awaiting_difficulty', 'player_turn_awaiting_emoji', 'player_score_18_plus_awaiting_choice',
            'p1_awaiting_roll1_emoji', 'p2_awaiting_roll1_emoji', 'p1_awaiting_roll_emoji', 'p2_awaiting_roll_emoji',
            'bot_rolling_internal', 'initiator_turn', 'opponent_turn', 'player_initial_roll_1_prompted',
            ...
            'bot_turn_pending_rolls', 'bot_rolling', 'awaiting_single_roll', 'processing_roll',
            'spinning_waiting_helper', 'player_turn', 'bot_turn', 'waiting_for_choice',
            'awaiting_player_choice', 'pvp_p1_chosen_wait_p2', 'pvp_p2_chosen_wait_p1'
        ];
        if (existingGameInstance && (nonTerminalStates.includes(existingGameInstance.status) || (existingGameInstance.status && !existingGameInstance.status.startsWith('game_over_')))) {
            await safeSendMessage(chatId, `⏳ Hold your hands, ${initiatorPlayerRefHTML}! An RPS game or offer is already active in this group. Please wait.`, { parse_mode: 'HTML' });
            return;
        } else if (!existingGameInstance) {
            console.warn(`${logPrefix} Stale RPS family lock (${RPS_FAMILY_LOCK_KEY}) found. Clearing.`);
    ...
    if (targetUserObject && targetUserObject.telegram_id) {
        // --- DIRECT PvP CHALLENGE OFFER for RPS ---
        ...
        offerData = {
            ...
            betAmount: betAmountLamports, originalGroupId: chatId, offerMessageIdInGroup: String(sentGroupMessage.message_id),
            chatTitle: msg.chat.title || `Group Chat ${chatId}`, status: 'pending_direct_challenge_response',
            gameToStart: GAME_IDS.RPS_PVP, creationTime: Date.now()
        };
    } else {
        // --- UNIFIED RPS OFFER ---
    ...
    activeGames.set(offerId, offerData);
    await updateGroupGameDetails(chatId, offerId, RPS_FAMILY_LOCK_KEY, betAmountLamports);
    console.log(`${logPrefix} Offer ${offerId} (Actual Type: ${offerData.type}) created. Group lock set for RPS family (${RPS_FAMILY_LOCK_KEY}) with ID: ${offerId}.`);

    setTimeout(async () => { // Common timeout logic for the offer
        const timedOutOffer = activeGames.get(offerId);
        if (timedOutOffer &&
            ((timedOutOffer.type === GAME_IDS.DIRECT_PVP_CHALLENGE && timedOutOffer.status === 'pending_direct_challenge_response') ||
             (timedOutOffer.type === GAME_IDS.RPS_UNIFIED_OFFER && (timedOutOffer.status === 'pending_offer' || timedOutOffer.status === 'awaiting_player_choice')))) {
            activeGames.delete(offerId);
            await updateGroupGameDetails(chatId, null, RPS_FAMILY_LOCK_KEY, null);
            // ... (message editing logic for timeout) ...
        }
...
    betsAlreadyDeducted = false // New flag
    ...
    // Determine chatId from initiatorData, which might be a full offer object or just user details
    const chatId = String(initiatorData.chatId || (initiatorData.initiatorUserObj ? initiatorData.initiatorUserObj.chat?.id : null) || originalChatId); // Fallback needed
    // If chatId is still null, this is an issue.
    // For now, assuming initiatorData.chatId is the primary source from unified offers or direct challenge offerData.

    const logPrefix = `[RPS_PvP_Start_NoDoubleBet_FamilyLockFix Init:${initiatorData.initiatorUserObj?.telegram_id || initiatorData.initiatorId} Join:${joinerUserObjFull.id || joinerUserObjFull.telegram_id} Chat:${chatId}]`;

    const betAmount = BigInt(initiatorData.betAmount);
    let initiatorUserObj = initiatorData.initiatorUserObj || initiatorData;
    const initiatorId = String(initiatorUserObj.id || initiatorUserObj.telegram_id);
    const initiatorMentionHTML = escapeHTML(getPlayerDisplayReference(initiatorUserObj));
    const joinerId = String(joinerUserObjFull.id || joinerUserObjFull.telegram_id);
    const joinerMentionHTML = escapeHTML(getPlayerDisplayReference(joinerUserObjFull));
...
        await bot.deleteMessage(chatId, Number(originalOfferMessageId)).catch(() => {});
    }
    if (initiatorData.gameId && (initiatorData.type === GAME_IDS.RPS_UNIFIED_OFFER || initiatorData.type === GAME_IDS.DIRECT_PVP_CHALLENGE) && activeGames.has(initiatorData.gameId)) {
        activeGames.delete(initiatorData.gameId);
        console.log(`${logPrefix} Deleted original offer ${initiatorData.gameId} as RPS PvP game starts.`);
    }

    let client;
    const pvpGameId = generateGameId(GAME_IDS.RPS_PVP);
    const RPS_FAMILY_LOCK_KEY = GAME_IDS.RPS_UNIFIED_OFFER; // Define for clarity
...
    activeGames.set(pvpGameId, gameDataPvP);
    // *** MODIFIED LINE FOR CANONICAL FAMILY ID LOCK ***
    await updateGroupGameDetails(chatId, pvpGameId, RPS_FAMILY_LOCK_KEY, betAmount);
    // *** END OF MODIFICATION ***
...
async function handleStartDiceEscalatorUnifiedOfferCommand_New(msg, betAmountLamports, targetUsernameRaw = null) {
    ...
    const chatId = String(msg.chat.id);
    const chatType = msg.chat.type;
    const logPrefix = `[DE_OfferOrDirect_V5_UserCap_SingleFamilyLock UID:${userId} CH:${chatId}]`; // Updated log

    // --- Global User Game Cap Check (remains the same) ---
    const activeUserGameCheck = await checkUserActiveGameLimit(userId);
    if (activeUserGameCheck.limitReached) {
        const gameTypeDisplay = activeUserGameCheck.details.type.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase());
    ...
    if (targetUsernameRaw) {
        ...
        if (!targetUserObjectForChallenge || !targetUserObjectForChallenge.telegram_id) {
            await safeSendMessage(chatId, `😕 Player ${escapeHTML(targetUsernameRaw)} not found. If you meant to make a general Dice Escalator offer, please use <code>/de ${escapeHTML(args[0] || "<bet>")}</code> without mentioning a user.`, { parse_mode: 'HTML' });
            return; // For direct challenge, target must be valid, otherwise user should re-issue as unified.
        } else if (String(targetUserObjectForChallenge.telegram_id) === userId) {
            await safeSendMessage(chatId, `😅 You can't directly challenge yourself, ${initiatorPlayerRefHTML}! To make a general offer, use <code>/de ${escapeHTML(args[0] || "<bet>")}</code>.`, { parse_mode: 'HTML' });
            return; // Prevent self-challenge for direct, guide to unified.
        }
    }

    // --- CORRECTED Per-Group, Per-Game-Family Lock Check ---
    const gameSession = await getGroupSession(chatId, msg.chat.title || `Group Chat ${chatId}`);
    // Use ONE canonical lock key for ALL Dice Escalator activities in this group
    const DE_FAMILY_LOCK_KEY = GAME_IDS.DICE_ESCALATOR_UNIFIED_OFFER; // Canonical key for this game family

    if (gameSession.activeGamesByTypeInGroup.has(DE_FAMILY_LOCK_KEY)) {
        const existingGameIdInGroup = gameSession.activeGamesByTypeInGroup.get(DE_FAMILY_LOCK_KEY);
        const existingGameInstance = activeGames.get(existingGameIdInGroup);
        ...
        const nonTerminalStates = [
            ...
            'pvp_p1_choosing', 'pvp_p2_choosing', 'pvb_waiting_player_choice', 'player_action_processing_stand',
            'bot_turn_pending_rolls', 'bot_rolling', 'awaiting_single_roll', 'processing_roll',
            'spinning_waiting_helper', 'player_turn', 'bot_turn' // Added generic turn states
        ];

        if (existingGameInstance && (nonTerminalStates.includes(existingGameInstance.status) || (existingGameInstance.status && !existingGameInstance.status.startsWith('game_over_')))) {
            let gamePhaseMessage = "A Dice Escalator game or offer";
            if (existingGameInstance.type === GAME_IDS.DIRECT_PVP_CHALLENGE) gamePhaseMessage = "A direct Dice Escalator challenge";
            else if (existingGameInstance.type === GAME_IDS.DICE_ESCALATOR_UNIFIED_OFFER) gamePhaseMessage = "A Dice Escalator offer";
            else if (existingGameInstance.type === GAME_IDS.DICE_ESCALATOR_PVB || existingGameInstance.type === GAME_IDS.DICE_ESCALATOR_PVP) gamePhaseMessage = "An active Dice Escalator game";

            await safeSendMessage(chatId, `⏳ Hold your dice, ${initiatorPlayerRefHTML}! ${gamePhaseMessage} is already in progress in this group (Game ID ending <code>...${escapeHTML(String(existingGameIdInGroup).slice(-6))}</code>, Status: <code>${escapeHTML(existingGameInstance.status)}</code>). Please wait.`, { parse_mode: 'HTML' });
            return;
        } else if (!existingGameInstance) {
            console.warn(`${logPrefix} Stale game ID ${existingGameIdInGroup} found in group session for Dice Escalator family lock (${DE_FAMILY_LOCK_KEY}). Clearing.`);
            await updateGroupGameDetails(chatId, null, DE_FAMILY_LOCK_KEY, null);
        }
    }
    // --- END CORRECTED Lock Check ---

    let offerId; // Will be the gameId for the offer object, and used to set the family lock.
    let offerData;

    if (targetUserObjectForChallenge && targetUserObjectForChallenge.telegram_id) {
        // --- DIRECT PvP CHALLENGE OFFER for Dice Escalator ---
        // The offer object itself is type DIRECT_PVP_CHALLENGE, but the lock it establishes is for DE_FAMILY_LOCK_KEY
        offerId = generateGameId(`dco_${userId.slice(-3)}_${String(targetUserObjectForChallenge.telegram_id).slice(-3)}`);
        ...
        // ... (DM notification logic unchanged) ...
        ...
        await safeSendMessage(targetUserObjectForChallenge.telegram_id, dmNotificationTextHTML, { parse_mode: 'HTML' });


        offerData = {
            type: GAME_IDS.DIRECT_PVP_CHALLENGE, // The type of this specific offer object
            offerId: offerId, gameId: offerId, initiatorId: userId,
            initiatorUserObj: initiatorUserObj, initiatorMentionHTML: initiatorPlayerRefHTML,
            targetUserId: String(targetUserObjectForChallenge.telegram_id),
            targetUserObj: targetUserObjectForChallenge, targetUserMentionHTML: targetPlayerRefHTML,
            betAmount: betAmountLamports, originalGroupId: chatId, offerMessageIdInGroup: offerMessageIdInGroup,
            chatTitle: msg.chat.title || `Group Chat ${chatId}`, status: 'pending_direct_challenge_response',
            gameToStart: GAME_IDS.DICE_ESCALATOR_PVP, // What game this direct challenge leads to
            creationTime: Date.now()
        };
        console.log(`${logPrefix} Direct Dice Escalator challenge offer object ${offerId} (type: DIRECT_PVP_CHALLENGE) created.`);
    } else {
        // --- DICE ESCALATOR UNIFIED OFFER ---
        // The offer object itself is type DICE_ESCALATOR_UNIFIED_OFFER, and this is also the family lock key
        ...
        if (!sentOfferMessage?.message_id) {
            await safeSendMessage(chatId, `⚙️ Oops! Couldn't create the Dice Escalator offer. Please try again.`, { parse_mode: 'HTML' });
            return;
        }
        offerData = {
            type: GAME_IDS.DICE_ESCALATOR_UNIFIED_OFFER, // The type of this specific offer object
            gameId: offerId, chatId: String(chatId),
            ...
            creationTime: Date.now(), gameSetupMessageId: String(sentOfferMessage.message_id),
        };
        console.log(`${logPrefix} Unified Dice Escalator offer object ${offerId} (type: DICE_ESCALATOR_UNIFIED_OFFER) created.`);
    }

    // Store the offer object in activeGames
    activeGames.set(offerId, offerData);
    // Set the group lock using the CANONICAL FAMILY KEY, pointing to this offer's ID
    await updateGroupGameDetails(chatId, offerId, DE_FAMILY_LOCK_KEY, betAmountLamports);
    console.log(`${logPrefix} Group lock set for Dice Escalator family (${DE_FAMILY_LOCK_KEY}) with game/offer ID: ${offerId}.`);

    // Common timeout logic for the offer object (whether direct or unified)
    setTimeout(async () => {
        const timedOutOffer = activeGames.get(offerId);
        // Check status based on the type of offerData that was created
        if (timedOutOffer &&
            ((timedOutOffer.type === GAME_IDS.DIRECT_PVP_CHALLENGE && timedOutOffer.status === 'pending_direct_challenge_response') ||
             (timedOutOffer.type === GAME_IDS.DICE_ESCALATOR_UNIFIED_OFFER && timedOutOffer.status === 'pending_unified_offer'))) {

            console.log(`${logPrefix} Offer ${offerId} (Type: ${timedOutOffer.type}) has timed out.`);
            activeGames.delete(offerId);
            // Clear the Dice Escalator family lock for the group
            await updateGroupGameDetails(chatId, null, DE_FAMILY_LOCK_KEY, null);

            let timeoutMsgHTML;
            let msgIdToEdit = timedOutOffer.offerMessageIdInGroup || timedOutOffer.gameSetupMessageId;

            if (timedOutOffer.type === GAME_IDS.DIRECT_PVP_CHALLENGE) {
                timeoutMsgHTML = `⏳ The Dice Escalator challenge from ${timedOutOffer.initiatorMentionHTML} to ${timedOutOffer.targetUserMentionHTML} for <b>${escapeHTML(await formatBalanceForDisplay(timedOutOffer.betAmount, 'USD'))}</b> has expired unanswered.`;
            ...
            } else { // DICE_ESCALATOR_UNIFIED_OFFER
                timeoutMsgHTML = `⏳ The Dice Escalator offer by ${timedOutOffer.initiatorMentionHTML} for <b>${escapeHTML(await formatBalanceForDisplay(timedOutOffer.betAmount, 'USD'))}</b> has expired unanswered.`;
            }

            if (msgIdToEdit && bot) {
                await bot.editMessageText(timeoutMsgHTML, {
                    chat_id: String(chatId), message_id: Number(msgIdToEdit),
                    parse_mode: 'HTML', reply_markup: {}
                }).catch(e => {
                    console.warn(`${logPrefix} Failed to edit expired DE offer message ${msgIdToEdit}: ${e.message}. Sending new.`);
                    safeSendMessage(String(chatId), timeoutMsgHTML, { parse_mode: 'HTML' });
                });
            } else if (!msgIdToEdit) { // Fallback if message ID wasn't stored
                 safeSendMessage(String(chatId), timeoutMsgHTML, { parse_mode: 'HTML' });
            }
        }
    }, JOIN_GAME_TIMEOUT_MS);
...
    let jackpotContribution = 0n;
    const DE_FAMILY_LOCK_KEY = GAME_IDS.DICE_ESCALATOR_UNIFIED_OFFER; // Define for clarity
...
    if (chat.type !== 'private') {
        // *** MODIFIED LINE FOR CANONICAL FAMILY ID LOCK ***
        await updateGroupGameDetails(chatId, gameId, DE_FAMILY_LOCK_KEY, betAmountLamports);
        // *** END OF MODIFICATION ***
    }
...
    console.log(`${logPrefix} New DE PvP game object (${pvpGameId}) created and stored in activeGames. Initial Game Status: '${gameData.status}', P1 turn: ${gameData.initiator.isTurn}`);
    ...
    // *** MODIFIED LINE FOR CANONICAL FAMILY ID LOCK ***
    const DE_FAMILY_LOCK_KEY = GAME_IDS.DICE_ESCALATOR_UNIFIED_OFFER;
    await updateGroupGameDetails(groupChatId, pvpGameId, DE_FAMILY_LOCK_KEY, betAmountLamports);
    // *** END OF MODIFICATION ***
    console.log(`${logPrefix} Group game details updated for chat ${groupChatId} to DE PvP game ${pvpGameId} using family lock key.`);
    
...
    } else {
        console.error(`${LOG_PREFIX_DE_OFFER_V5_SHORTCB} CRITICAL ERROR: updateDiceEscalatorPvPMessage_New function is not defined! Cannot display game board for ${pvpGameId}.`); // Note: LOG_PREFIX_DE_OFFER_V5_SHORTCB seems like a copy-paste, should be logPrefix
...
    const chatId = String(msg.chat.id);
    const chatType = msg.chat.type;
    const LOG_PREFIX_D21_START_HTML = `[D21_OfferOrDirect_UserCap_CanonLock_V2 UID:${userId} CH:${chatId} Type:${chatType}]`;
...
        targetUserObject = await findRecipientUser(targetUsernameRaw);
        if (!targetUserObject || !targetUserObject.telegram_id) {
            await safeSendMessage(chatId, `😕 Player ${escapeHTML(targetUsernameRaw)} not found. To make a general Dice 21 offer, use <code>/d21 ${escapeHTML(args[0] || "<bet>")}</code>.`, { parse_mode: 'HTML' });
            return;
        } else if (String(targetUserObject.telegram_id) === userId) {
            await safeSendMessage(chatId, `😅 You can't directly challenge yourself, ${initiatorPlayerRefHTML}! To make a general offer, use <code>/d21 ${escapeHTML(args[0] || "<bet>")}</code>.`, { parse_mode: 'HTML' });
            return;
        }
    }

    const gameSession = await getGroupSession(chatId, msg.chat.title || `Group Chat ${chatId}`);
    const DICE21_FAMILY_LOCK_KEY = GAME_IDS.DICE_21_UNIFIED_OFFER;

    if (gameSession.activeGamesByTypeInGroup.has(DICE21_FAMILY_LOCK_KEY)) {
        const existingGameIdInGroup = gameSession.activeGamesByTypeInGroup.get(DICE21_FAMILY_LOCK_KEY);
        const existingGameInstance = activeGames.get(existingGameIdInGroup);
...
        if (existingGameInstance && (nonTerminalStates.includes(existingGameInstance.status) || (existingGameInstance.status && !existingGameInstance.status.startsWith('game_over_')))) {
            ...
            const gameNameDisplay = "Dice 21";
            let activityDescription;
            if (existingGameInstance.type === GAME_IDS.DICE_21_UNIFIED_OFFER ||
                (existingGameInstance.type === GAME_IDS.DIRECT_PVP_CHALLENGE && existingGameInstance.gameToStart === GAME_IDS.DICE_21_PVP)) {
                activityDescription = `an offer for a ${gameNameDisplay} game is currently active`;
            } else { // Assumes it's an active game like GAME_IDS.DICE_21 or GAME_IDS.DICE_21_PVP
                activityDescription = `a ${gameNameDisplay} game is currently in progress`;
            }
            await safeSendMessage(chatId, `⏳ Please hold on, ${initiatorPlayerRefHTML}! It looks like ${activityDescription} in this group.<br>Kindly wait for it to conclude before starting a new one.`, { parse_mode: 'HTML' });
            // *** END OF MODIFIED MESSAGE ***
            return;
        } else if (!existingGameInstance) {
            console.warn(`${LOG_PREFIX_D21_START_HTML} Stale game ID ${existingGameIdInGroup} found in group session for Dice 21 family lock (${DICE21_FAMILY_LOCK_KEY}). Clearing.`);
...
        console.log(`${LOG_PREFIX_D21_START_HTML} Initiating DIRECT Dice 21 challenge (Offer ID: ${offerId}) to User ID: ${targetUserObject.telegram_id}`);
        const targetPlayerRefHTML = escapeHTML(getPlayerDisplayReference(targetUserObject));
...
        await safeSendMessage(targetUserObject.telegram_id, dmNotificationTextHTML, { parse_mode: 'HTML' });
...
            betAmount: betAmountLamports, originalGroupId: chatId, offerMessageIdInGroup: offerMessageIdInGroup,
            chatTitle: msg.chat.title || `Group Chat ${chatId}`, status: 'pending_direct_challenge_response',
            gameToStart: GAME_IDS.DICE_21_PVP, creationTime: Date.now()
        };
    } else {
        offerId = generateGameId(GAME_IDS.DICE_21_UNIFIED_OFFER);
        console.log(`${LOG_PREFIX_D21_START_HTML} Initiating UNIFIED Dice 21 offer (Offer ID: ${offerId}).`);
        const offerMessageTextHTML =
...
        if (!sentOfferMessage?.message_id) { /* ... error handling ... */ return; }
        offerData = {
...
    activeGames.set(offerId, offerData);
    await updateGroupGameDetails(chatId, offerId, DICE21_FAMILY_LOCK_KEY, betAmountLamports);
    console.log(`${LOG_PREFIX_D21_START_HTML} Offer ${offerId} (Type: ${offerData.type}) created. Group lock set for Dice 21 family (${DICE21_FAMILY_LOCK_KEY}) with ID: ${offerId}.`);

    setTimeout(async () => {
        const timedOutOffer = activeGames.get(offerId);
        if (timedOutOffer &&
            ((timedOutOffer.type === GAME_IDS.DIRECT_PVP_CHALLENGE && timedOutOffer.status === 'pending_direct_challenge_response') ||
             (timedOutOffer.type === GAME_IDS.DICE_21_UNIFIED_OFFER && timedOutOffer.status === 'waiting_for_choice'))) {

            console.log(`[D21_OfferTimeout OfferID:${offerId}] Offer (Type: ${timedOutOffer.type}) has expired.`);
            activeGames.delete(offerId);
            await updateGroupGameDetails(chatId, null, DICE21_FAMILY_LOCK_KEY, null);
...
            if (timedOutOffer.type === GAME_IDS.DIRECT_PVP_CHALLENGE) {
                timeoutMsgHTML = `⏳ The Dice 21 challenge from ${timedOutOffer.initiatorMentionHTML} to ${timedOutOffer.targetUserMentionHTML} for <b>${escapeHTML(await formatBalanceForDisplay(timedOutOffer.betAmount, 'USD'))}</b> has expired unanswered.`;
                if (timedOutOffer.initiatorId) {
...
                }
            } else { // DICE_21_UNIFIED_OFFER
                timeoutMsgHTML = `⏳ The Dice 21 game offer initiated by ${timedOutOffer.initiatorMention} for <b>${escapeHTML(await formatBalanceForDisplay(timedOutOffer.betAmount, 'USD'))}</b> has timed out as no option was selected. This offer is now closed.`;
            }
...
                    parse_mode: 'HTML', reply_markup: {}
                }).catch(e => {
                    console.warn(`${LOG_PREFIX_D21_START_HTML} Failed to edit expired D21 offer message ${msgIdToEdit}: ${e.message}. Sending new if applicable.`);
                    if(!msgIdToEdit && timedOutOffer.type === GAME_IDS.DIRECT_PVP_CHALLENGE) {
                         safeSendMessage(String(chatId), timeoutMsgHTML, { parse_mode: 'HTML' });
                    }
                });
            } else if (!msgIdToEdit && timedOutOffer.type === GAME_IDS.DIRECT_PVP_CHALLENGE){
                 safeSendMessage(String(chatId), timeoutMsgHTML, { parse_mode: 'HTML' });
            }
        }
    }, JOIN_GAME_TIMEOUT_MS);
...
        }
        activeGames.delete(offerId);
       // Ensure the correct family lock key is used for clearing if game offer is cancelled
       const DICE21_FAMILY_LOCK_KEY_FOR_CANCEL = GAME_IDS.DICE_21_UNIFIED_OFFER;
        await updateGroupGameDetails(originalChatId, null, DICE21_FAMILY_LOCK_KEY_FOR_CANCEL, null);
        return;
    }
...
        activeGames.delete(offerId);
        // *** MODIFIED LINE FOR CANONICAL FAMILY ID LOCK ***
        const DICE21_FAMILY_LOCK_KEY = GAME_IDS.DICE_21_UNIFIED_OFFER;
        await updateGroupGameDetails(originalChatId, pvpGameId, DICE21_FAMILY_LOCK_KEY, betAmount);
        // *** END OF MODIFICATION ***
...
        if (activeGames.has(pvpGameId)) activeGames.delete(pvpGameId);
       // Ensure the correct family lock key is used for clearing if game creation fails after lock was potentially set
       const DICE21_FAMILY_LOCK_KEY_FOR_ERROR_CLEAR = GAME_IDS.DICE_21_UNIFIED_OFFER;
        await updateGroupGameDetails(originalChatId, null, DICE21_FAMILY_LOCK_KEY_FOR_ERROR_CLEAR, null);
...
            // *** MODIFIED LINE FOR CANONICAL FAMILY ID LOCK ***
            const DICE21_FAMILY_LOCK_KEY = GAME_IDS.DICE_21_UNIFIED_OFFER;
            await updateGroupGameDetails(chatId, gameIdForActivePvB, DICE21_FAMILY_LOCK_KEY, betAmountLamports);
            // *** END OF MODIFICATION ***
        }
...
        if (currentChatType !== 'private') {
          // ***Ensure the correct family lock key is used for clearing if game setup fails after lock was potentially set***
          const DICE21_FAMILY_LOCK_KEY = GAME_IDS.DICE_21_UNIFIED_OFFER;
          await updateGroupGameDetails(chatId, null, DICE21_FAMILY_LOCK_KEY, null);
        }
    } finally {
...
async function finalizeDice21PvPGame(gameData) {
    // --- BEGIN DIAGNOSTIC LOGS (Kept for initial verification if needed, can be removed later) ---
    if (!gameData) {
        console.error(`[D21_PvP_Finalize_DIAGNOSTIC] CRITICAL: gameData object itself is null or undefined when passed to finalizeDice21PvPGame.`);
        return;
    }
    const diagnosticLogPrefix = `[D21_PvP_Finalize_DIAGNOSTIC GID:${gameData.gameId || 'UNKNOWN_GAME_ID'}]`;
    // console.log(`${diagnosticLogPrefix} Entered finalizeDice21PvPGame. Full gameData received (abbreviated if long): ${stringifyWithBigInt(gameData).substring(0, 1500)}`); // Can be very verbose

    if (!gameData.initiator) {
        console.error(`${diagnosticLogPrefix} CRITICAL_ERROR: gameData.initiator is undefined!`);
        throw new Error("CRITICAL_ERROR_FINALIZE_PVP_D21: gameData.initiator is undefined.");
    } else if (typeof gameData.initiator.score === 'undefined') {
        console.error(`${diagnosticLogPrefix} CRITICAL_ERROR: gameData.initiator.score is undefined! Initiator object: ${stringifyWithBigInt(gameData.initiator)}`);
        throw new Error("CRITICAL_ERROR_FINALIZE_PVP_D21: gameData.initiator.score is undefined.");
    }

    if (!gameData.opponent) {
        console.error(`${diagnosticLogPrefix} CRITICAL_ERROR: gameData.opponent is undefined!`);
        throw new Error("CRITICAL_ERROR_FINALIZE_PVP_D21: gameData.opponent is undefined.");
    } else if (typeof gameData.opponent.score === 'undefined') {
        console.error(`${diagnosticLogPrefix} CRITICAL_ERROR: gameData.opponent.score is undefined! Opponent object: ${stringifyWithBigInt(gameData.opponent)}`);
        throw new Error("CRITICAL_ERROR_FINALIZE_PVP_D21: gameData.opponent.score is undefined.");
    }
    // --- END DIAGNOSTIC LOGS ---
...
    const finalStatus = gameData.status;
    ...
    const { gameId, chatId, betAmount, initiator, opponent, chatType } = gameData;
    const p1 = initiator; // Assign to p1 if rest of function uses p1
    const p2 = opponent;  // Assign to p2 if rest of function uses p2
    // *** END OF MODIFICATION ***

    // Defensive check after assignment, though diagnostics above should catch it if initiator/opponent were initially issues
    if (!p1 || typeof p1.score === 'undefined') {
        console.error(`${logPrefix} CRITICAL after mapping: p1 or p1.score is undefined. p1: ${stringifyWithBigInt(p1)}`);
        throw new Error("CRITICAL_ERROR_FINALIZE_PVP_D21: p1 or p1.score became undefined after mapping.");
    }
    if (!p2 || typeof p2.score === 'undefined') {
        console.error(`${logPrefix} CRITICAL after mapping: p2 or p2.score is undefined. p2: ${stringifyWithBigInt(p2)}`);
        throw new Error("CRITICAL_ERROR_FINALIZE_PVP_D21: p2 or p2.score became undefined after mapping.");
    }
...
        p2_payout = gameData.betAmount * 2n; // Use gameData.betAmount here as betAmount is from destructuring
...
        p1_payout = gameData.betAmount * 2n; // Use gameData.betAmount here
...
        const determineLedgerType = (payout, betValue, isPushOrError, isBlackjackWin = false, isWinByForfeit = false) => { // Renamed bet to betValue to avoid conflict
...
            return payout > betValue ? 'win_dice21_pvp' : (payout === 0n ? 'loss_dice21_pvp' : 'unknown_dice21_pvp_outcome');
        };
...
    if (criticalDbErrorForAdmin && typeof notifyAdmin === 'function') {
        notifyAdmin(`🚨 CRITICAL D21 PvP Payout Failure 🚨\nGame ID: <code>${escapeHTML(gameData.gameId)}</code>\nError: ${escapeHTML(dbErrorTextForUserHTML.includes("DB Error") ? "DB Error during payout" : "Unknown payout error" )}. MANUAL CHECK REQUIRED.`, {parse_mode: 'HTML'});
    }
...
        console.error(`${LOG_PREFIX_OU7_START} Database error during Over/Under 7 bet processing: ${dbError.message}`, dbError.stack?.substring(0, 500));
            ...
            if (currentTimedOutGame && currentTimedOutGame.status === 'waiting_player_choice') {
                ...
                if (bot && currentTimedOutGame.gameMessageId) {
                    await bot.editMessageText(timeoutMsg, {
                        chat_id: chatId,
                        message_id: Number(currentTimedOutGame.gameMessageId),
                        parse_mode: 'HTML',
...
    const userId = String(userObj.telegram_id);
    const chatId = String(msgContext.chatId || originalMessageIdFromCallback); // Ensure chatId is available
    const LOG_PREFIX_OU7_CHOICE = `[OU7_Choice_TimeoutClear UID:${userId} GID:${gameId}]`; // Updated log
...
    }
    ...
    if (gameData.timeoutId) {
        ...
        console.log(`${LOG_PREFIX_OU7_CHOICE} Choice timeout cleared for game ${gameId}.`);
    }
...
    const { playerRef, betAmount } = gameData; // Removed chatId from here as it's now a parameter
...
    let currentMessageId = gameData.gameMessageId; // This is the ID of the message with choice buttons
...
            await bot.editMessageText(rollingMessageTextHTML, { chat_id: String(chatId), message_id: Number(currentMessageId), parse_mode: 'HTML', reply_markup: {} });
            // gameData.gameMessageId remains the same, as we are editing it.
        } catch (e) {
            if (!e.message || !e.message.toLowerCase().includes("message is not modified")) {
                console.warn(`${LOG_PREFIX_OU7_CHOICE} Failed to edit message ${currentMessageId} to 'rolling dice', sending new. Error: ${e.message}`);
...
                if (newMsg?.message_id && activeGames.has(gameId)) { // Check if gameData still exists
                    ...
                    if (currentGd) {
                        currentGd.gameMessageId = newMsg.message_id; // Update gameMessageId if new message sent
                        activeGames.set(gameId, currentGd);
                        currentMessageId = newMsg.message_id; // Update local currentMessageId for this scope
                    }
                }
            }
        }
    } else { // Should not happen if gameMessageId was set in start command
        console.warn(`${LOG_PREFIX_OU7_CHOICE} gameMessageId was null when trying to update to 'rolling dice'. Sending new.`);
        const newMsg = await safeSendMessage(String(chatId), rollingMessageTextHTML, { parse_mode: 'HTML' });
...
    }
    ...
    // The message to delete before sending the final result is the one that said "rolling dice..."
...
    gameData.status = 'game_over';
    // gameData.gameMessageId should now hold the ID of the "rolling dice..." message.
    activeGames.set(gameId, gameData); // Save updates before finalizing
...
async function handleStartDuelUnifiedOfferCommand(msg, betAmountLamports, targetUsernameRaw = null) {
    ...
    const chatId = String(msg.chat.id);
    const chatType = msg.chat.type;
    const logPrefix = `[Duel_OfferOrDirect_UserCap_CanonLock_V3 UID:${userId} CH:${chatId} Type:${chatType}]`; // Updated log

    // --- BEGIN MODIFICATION: Global User Game Cap ---
    const activeUserGameCheck = await checkUserActiveGameLimit(userId);
    if (activeUserGameCheck.limitReached) {
        const gameTypeDisplay = activeUserGameCheck.details.type.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase());
    ...
    // --- END MODIFICATION: Global User Game Cap ---
    ...
    if (targetUsernameRaw) {
        targetUserObject = await findRecipientUser(targetUsernameRaw);
        if (!targetUserObject || !targetUserObject.telegram_id) {
            await safeSendMessage(chatId, `😕 Player ${escapeHTML(targetUsernameRaw)} not found. To make a general Duel offer, use <code>/duel ${escapeHTML(args[0] || "<bet>")}</code>.`, { parse_mode: 'HTML' });
            return; // For direct challenge, target must be valid.
        } else if (String(targetUserObject.telegram_id) === userId) {
            await safeSendMessage(chatId, `😅 You can't challenge yourself to a Duel, ${initiatorPlayerRefHTML}! To make a general offer, use <code>/duel ${escapeHTML(args[0] || "<bet>")}</code>.`, { parse_mode: 'HTML' });
            return; // Prevent self-challenge for direct.
        }
    }

    // --- CORRECTED Per-Group, Per-Game-Family Lock Check ---
    const gameSession = await getGroupSession(chatId, msg.chat.title || `Group Chat ${chatId}`);
    // Use ONE canonical lock key for ALL Duel activities in this group
    const DUEL_FAMILY_LOCK_KEY = GAME_IDS.DUEL_UNIFIED_OFFER;

    if (gameSession.activeGamesByTypeInGroup.has(DUEL_FAMILY_LOCK_KEY)) {
        const existingGameIdInGroup = gameSession.activeGamesByTypeInGroup.get(DUEL_FAMILY_LOCK_KEY);
        const existingGameInstance = activeGames.get(existingGameIdInGroup);

        const nonTerminalStates = [ /* ... (comprehensive list of non-terminal game/offer states) ... */
            'pending_offer', 'pending_unified_offer', 'pending_direct_challenge_response',
            'awaiting_difficulty', 'player_turn_awaiting_emoji', 'player_score_18_plus_awaiting_choice',
            'p1_awaiting_roll1_emoji', 'p2_awaiting_roll1_emoji', 'p1_awaiting_roll_emoji', 'p2_awaiting_roll_emoji',
            'bot_rolling_internal', 'initiator_turn', 'opponent_turn', 'player_initial_roll_1_prompted',
        ...
        if (existingGameInstance && (nonTerminalStates.includes(existingGameInstance.status) || (existingGameInstance.status && !existingGameInstance.status.startsWith('game_over_')))) {
            let gamePhaseMessage = "A Duel game or offer";
             if (existingGameInstance.type === GAME_IDS.DIRECT_PVP_CHALLENGE && existingGameInstance.gameToStart === GAME_IDS.DUEL_PVP) gamePhaseMessage = "A direct Duel challenge";
            else if (existingGameInstance.type === GAME_IDS.DUEL_UNIFIED_OFFER) gamePhaseMessage = "A Duel offer";
            else if (existingGameInstance.type === GAME_IDS.DUEL_PVB || existingGameInstance.type === GAME_IDS.DUEL_PVP) gamePhaseMessage = "An active Duel game";


            await safeSendMessage(chatId, `⏳ Hold your weapons, ${initiatorPlayerRefHTML}! ${gamePhaseMessage} is already active in this group (Game ID ending <code>...${escapeHTML(String(existingGameIdInGroup).slice(-6))}</code>, Status: <code>${escapeHTML(existingGameInstance.status)}</code>). Please wait.`, { parse_mode: 'HTML' });
            return;
        } else if (!existingGameInstance) {
            console.warn(`${logPrefix} Stale game ID ${existingGameIdInGroup} found in group session for Duel family lock (${DUEL_FAMILY_LOCK_KEY}). Clearing.`);
            await updateGroupGameDetails(chatId, null, DUEL_FAMILY_LOCK_KEY, null);
        }
    }
    // --- END CORRECTED Lock Check ---

    let offerId; // ID of the offer object being created
    let offerData; // Data for the offer object

    if (targetUserObject && targetUserObject.telegram_id) {
        // --- DIRECT PvP CHALLENGE OFFER for Duel ---
        // The offer object in activeGames is type DIRECT_PVP_CHALLENGE.
        // The lock in gameSession.activeGamesByTypeInGroup uses DUEL_FAMILY_LOCK_KEY.
        offerId = generateGameId(`dduel_${userId.slice(-3)}_${String(targetUserObject.telegram_id).slice(-3)}`);
        console.log(`${logPrefix} Initiating DIRECT Duel challenge (Offer ID: ${offerId}) to User ID: ${targetUserObject.telegram_id}`);
        const targetPlayerRefHTML = escapeHTML(getPlayerDisplayReference(targetUserObject));

        const groupChallengeTextHTML = `Hey ${targetPlayerRefHTML}❗\n\n${initiatorPlayerRefHTML} has thrown down the gauntlet, challenging you to a <b>Duel</b> for <b>${betDisplayUSD_HTML}</b>!`;
        const groupChallengeKeyboard = { /* ... existing keyboard ... */
            inline_keyboard: [
                [{ text: "✅ Accept Challenge", callback_data: `dir_chal_acc:${offerId}` }],
                [{ text: "❌ Decline Challenge", callback_data: `dir_chal_dec:${offerId}` }],
                [{ text: "🚫 Withdraw My Challenge", callback_data: `dir_chal_can:${offerId}` }]
        ...
        offerData = {
            ...
            betAmount: betAmountLamports, originalGroupId: chatId, offerMessageIdInGroup: offerMessageIdInGroup,
            chatTitle: msg.chat.title || `Group Chat ${chatId}`, status: 'pending_direct_challenge_response',
            gameToStart: GAME_IDS.DUEL_PVP, creationTime: Date.now()
        };
    } else {
        // --- DUEL UNIFIED OFFER ---
        // The offer object in activeGames is type DUEL_UNIFIED_OFFER.
        // The lock in gameSession.activeGamesByTypeInGroup also uses DUEL_FAMILY_LOCK_KEY.
        offerId = generateGameId(GAME_IDS.DUEL_UNIFIED_OFFER);
        console.log(`${logPrefix} Initiating UNIFIED Duel offer (Offer ID: ${offerId}).`);
        const offerMessageTextHTML = /* ... as before ... */
        ...
        offerData = {
            ...
            betAmount: betAmountLamports, status: 'waiting_for_choice', creationTime: Date.now(),
            gameSetupMessageId: String(sentOfferMessage.message_id)
        };
    }

    // Store the offer object in activeGames
    activeGames.set(offerId, offerData);
    // Set the group lock using the CANONICAL FAMILY KEY for Duel, pointing to this new offer's ID
    await updateGroupGameDetails(chatId, offerId, DUEL_FAMILY_LOCK_KEY, betAmountLamports);
    console.log(`${logPrefix} Offer ${offerId} (Actual Type: ${offerData.type}) created. Group lock set for Duel family (${DUEL_FAMILY_LOCK_KEY}) with ID: ${offerId}.`);

    // Common timeout logic for the offer object
    setTimeout(async () => {
        const timedOutOffer = activeGames.get(offerId);
        if (timedOutOffer &&
            ((timedOutOffer.type === GAME_IDS.DIRECT_PVP_CHALLENGE && timedOutOffer.status === 'pending_direct_challenge_response') ||
             (timedOutOffer.type === GAME_IDS.DUEL_UNIFIED_OFFER && timedOutOffer.status === 'waiting_for_choice'))) {

            console.log(`${logPrefix} Offer ${offerId} (Type: ${timedOutOffer.type}) for Duel has timed out.`);
            activeGames.delete(offerId);
            await updateGroupGameDetails(chatId, null, DUEL_FAMILY_LOCK_KEY, null); // Clear Duel family lock

            let timeoutMsgHTML;
            let msgIdToEdit = timedOutOffer.offerMessageIdInGroup || timedOutOffer.gameSetupMessageId;

            if (timedOutOffer.type === GAME_IDS.DIRECT_PVP_CHALLENGE) {
                timeoutMsgHTML = `⏳ The Duel challenge from ${timedOutOffer.initiatorMentionHTML} to ${timedOutOffer.targetUserMentionHTML} for <b>${escapeHTML(await formatBalanceForDisplay(timedOutOffer.betAmount, 'USD'))}</b> has expired unanswered.`;
                if (timedOutOffer.initiatorId) {
            ...
            } else { // DUEL_UNIFIED_OFFER
                timeoutMsgHTML = `⏳ The Duel challenge by ${timedOutOffer.initiatorMention} for <b>${escapeHTML(await formatBalanceForDisplay(timedOutOffer.betAmount, 'USD'))}</b> has timed out. This challenge is now closed.`;
            }
            ...
            if (msgIdToEdit && bot) {
                await bot.editMessageText(timeoutMsgHTML, {
                    ...
                    console.warn(`${logPrefix} Failed to edit expired Duel offer message ${msgIdToEdit}: ${e.message}. Sending new if applicable.`);
                    if(!msgIdToEdit && timedOutOffer.type === GAME_IDS.DIRECT_PVP_CHALLENGE) {
                         safeSendMessage(String(chatId), timeoutMsgHTML, { parse_mode: 'HTML' });
                    }
                });
...
    const joinerRef = getPlayerDisplayReference(joinerUserObjFromCb);
    const callbackQueryId = callbackQueryIdPassed;
    const DUEL_FAMILY_LOCK_KEY = GAME_IDS.DUEL_UNIFIED_OFFER; // Define for clarity
...
        activeGames.delete(offerId);
        // Ensure correct family key used on error cleanup for group lock
        await updateGroupGameDetails(originalChatId, null, DUEL_FAMILY_LOCK_KEY, null);
        return;
    }
...
        activeGames.delete(offerId);
        // *** MODIFIED LINE FOR CANONICAL FAMILY ID LOCK ***
        await updateGroupGameDetails(originalChatId, pvpGameId, DUEL_FAMILY_LOCK_KEY, betAmount);
        // *** END OF MODIFICATION ***

        await startDuelPvPGameSequence(pvpGameId);
...
        if (currentBalance === null || BigInt(currentBalance) < betAmountLamports) {
...
    activeGames.set(gameId, gameData);
    // *** MODIFIED LINE FOR CANONICAL FAMILY ID LOCK ***
    await updateGroupGameDetails(chatId, gameId, DUEL_FAMILY_LOCK_KEY, betAmountLamports);
    // *** END OF MODIFICATION ***

    const initialMessageText =
        `⚔️ **Duel vs. Bot Dealer!** ⚔️\n\n` +
        `${playerRef}, your wager: *${betDisplayUSD}*.\n\n` +
        `It's your turn to roll! Please send your **first** 🎲 dice emoji.`;
        
...
async function handleStartLadderCommand(msg, betAmountLamports) {
    const userId = String(msg.from.id || msg.from.telegram_id);
    const chatId = String(msg.chat.id);
    const LOG_PREFIX_LADDER_START = `[Ladder_Start_HTML_V4_DeleteSend_UserCap UID:${userId} CH:${chatId}]`; // Updated log

    // --- BEGIN MODIFICATION: Global User Game Cap ---
    const activeGameCheck = await checkUserActiveGameLimit(userId);
    if (activeGameCheck.limitReached) {
        const gameTypeDisplay = activeGameCheck.details.type.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase());
        await safeSendMessage(chatId, `${escapeHTML(getPlayerDisplayReference(msg.from))}, you already have an active game of "${escapeHTML(gameTypeDisplay)}" (Status: ${escapeHTML(activeGameCheck.details.status)}). Please finish or resolve it before starting a new one.`, { parse_mode: 'HTML' });
        return;
    }
    // --- END MODIFICATION: Global User Game Cap ---
    ...
    // --- MODIFICATION: Group Lock Check Removed ---
    // No check for gameSession.currentGameId here
    ...
    // --- MODIFICATION: updateGroupGameDetails call removed (or modified not to lock) ---
    // This game type should not set gameSession.currentGameId.
    ...
    if (sentRollingMsg?.message_id) {
        // Re-fetch gameData before modifying
        const currentData = activeGames.get(gameId);
        if(currentData) {
            currentData.gameMessageId = sentRollingMsg.message_id;
            activeGames.set(gameId, currentData);
            console.log(`${LOG_PREFIX_LADDER_START} 'Climb Begins' message sent. Msg ID: ${currentData.gameMessageId}`);
        } else {
             console.error(`${LOG_PREFIX_LADDER_START} GameData for ${gameId} disappeared after sending initial message. This should not happen.`);
        }
    } else {
        ...
        try {
            ...
            if(refundClient) await refundClient.query('ROLLBACK');
            console.error(`${LOG_PREFIX_LADDER_START} Refund FAILED for game ${gameId}: ${e.message}`);
        } finally { if(refundClient) refundClient.release(); }
        activeGames.delete(gameId);
        return;
    }
    ...
    // Re-fetch gameData before updating, as async operations (Helper Bot calls) happened
    const currentDataAfterRolls = activeGames.get(gameId);
    if(!currentDataAfterRolls){
        console.error(`${LOG_PREFIX_LADDER_START} GameData for ${gameId} disappeared after dice rolls. Cannot finalize.`);
        return;
    }
    currentDataAfterRolls.rolls = diceRolls;
    currentDataAfterRolls.sum = BigInt(diceRolls.reduce((sum, val) => sum + val, 0));
    // No need to activeGames.set here if currentDataAfterRolls is a direct reference.

    if (helperBotError || (!isBust && currentDataAfterRolls.rolls.length !== LADDER_ROLL_COUNT)) {
        console.error(`${LOG_PREFIX_LADDER_START} Helper Bot error or incorrect roll count (and not a player bust). Error: ${helperBotError}, Rolls: ${currentDataAfterRolls.rolls.length}`);
        ...
        if (currentDataAfterRolls.gameMessageId && bot) {
            await bot.deleteMessage(String(chatId), Number(currentDataAfterRolls.gameMessageId)).catch(e => console.warn(`${LOG_PREFIX_LADDER_START} Non-critical: Failed to delete old message ${currentDataAfterRolls.gameMessageId} on helper error. Error: ${e.message}`));
        }
    ...
    console.log(`${LOG_PREFIX_LADDER_START} All dice rolls processed (or bust occurred). Final rolls: ${currentDataAfterRolls.rolls.join(', ')}, Sum: ${currentDataAfterRolls.sum}, isBust: ${isBust}`);
    ...
    let finalMessageTextHTML;
    try {
        finalMessageTextHTML = `${titleResultHTML}\n\n${playerRefHTML}'s wager: <b>${betDisplayUSD_HTML}</b>\nThe Helper Bot delivered dice: ${formatDiceRolls(currentDataAfterRolls.rolls)}\nTotal Sum: <b>${escapeHTML(String(currentDataAfterRolls.sum))}</b>\n\n`;
        ...
        if (currentDataAfterRolls.gameMessageId && bot) await bot.deleteMessage(String(chatId), Number(currentDataAfterRolls.gameMessageId)).catch(()=>{});
    ...
    try {
        ...
        clientOutcome = await pool.connect();
        await clientOutcome.query('BEGIN');
        const ledgerNotes = `Greed's Ladder: Sum ${currentDataAfterRolls.sum}, Rolls ${currentDataAfterRolls.rolls.join(',')}. Outcome: ${outcomeReasonLog}. GameID: ${gameId}`;
        const balanceUpdate = await updateUserBalanceAndLedger(
    ...
    console.log(`${LOG_PREFIX_LADDER_START} DB operations complete. Preparing to display final result.`);

    const postGameKeyboardLadder = createPostGameKeyboard(GAME_IDS.LADDER, betAmountLamports);

    if (currentDataAfterRolls.gameMessageId && bot) {
        console.log(`${LOG_PREFIX_LADDER_START} Attempting to DELETE old message ID: ${currentDataAfterRolls.gameMessageId}`);
        await bot.deleteMessage(String(chatId), Number(currentDataAfterRolls.gameMessageId))
            .catch(e => console.warn(`${LOG_PREFIX_LADDER_START} Non-critical: Failed to delete old message ${currentDataAfterRolls.gameMessageId}. Error: ${e.message}`));
    } else {
...
async function handleStartSevenOutCommand(msg, betAmountLamports) {
    const userId = String(msg.from.id || msg.from.telegram_id);
    const chatId = String(msg.chat.id);
    const LOG_PREFIX_S7_LUCKY_SUM_START = `[S7_LuckySum_Start_HTML_Fix_UserCap UID:${userId} CH:${chatId}]`; // Updated log
    ...
    const gameId = generateGameId(GAME_IDS.SEVEN_OUT); // GAME_IDS.SEVEN_OUT is used for Lucky Sum
    let client;
    try {
        client = await pool.connect();
        await client.query('BEGIN');
    ...
    const gameData = {
        ...
        status: 'awaiting_single_roll', // This will trigger processSevenOutRoll
        gameMessageIdToDelete: null, // For the "rolling..." message
        lastInteractionTime: Date.now()
    };
    activeGames.set(gameId, gameData);
    ...
    if (sentInitialMsg?.message_id) {
        // Re-fetch gameData from activeGames before modifying
        const currentData = activeGames.get(gameId);
        if(currentData) {
            currentData.gameMessageIdToDelete = sentInitialMsg.message_id; // Store to delete after dice are "rolled"
            activeGames.set(gameId, currentData);
        }
        // Call processSevenOutRoll to handle the dice rolling and game finalization
        // It is assumed that processSevenOutRoll and finalizeSevenOutGame are correct as per your Part 5c, Section 3.
        if (typeof processSevenOutRoll === 'function') {
            await processSevenOutRoll(gameData); // Pass the initially created gameData
        } else {
            console.error(`${LOG_PREFIX_S7_LUCKY_SUM_START} CRITICAL: processSevenOutRoll function not found!`);
            // Handle error, potentially refund
            activeGames.delete(gameId);
            // (Error handling and refund logic if processSevenOutRoll is missing would go here)
        }
    } else {
...
async function handleStartSlotCommand(msg, betAmountLamports) {
    const userId = String(msg.from.id || msg.from.telegram_id);
    const chatId = String(msg.chat.id);
    const LOG_PREFIX_SLOT_START = `[Slot_Start_HTML_NewMsg_UserCap UID:${userId} CH:${chatId}]`; // Updated log

    // --- BEGIN MODIFICATION: Global User Game Cap ---
    const activeGameCheck = await checkUserActiveGameLimit(userId);
    if (activeGameCheck.limitReached) {
        const gameTypeDisplay = activeGameCheck.details.type.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase());
        // Using safeSendMessage and HTML as per existing style in this handler
        await safeSendMessage(chatId, `${escapeHTML(getPlayerDisplayReference(msg.from))}, you already have an active game of "${escapeHTML(gameTypeDisplay)}" (Status: ${escapeHTML(activeGameCheck.details.status)}). Please finish or resolve it before starting a new one.`, { parse_mode: 'HTML' });
        return;
    }
    // --- END MODIFICATION: Global User Game Cap ---
    ...
    if (sentSpinningMsg?.message_id) {
        // Re-fetch gameData from activeGames before modifying, to ensure it's the instance in the map
        const currentData = activeGames.get(gameId);
        if (currentData) {
            currentData.gameMessageId = sentSpinningMsg.message_id;
            // No need to activeGames.set again if currentData is a direct reference, but safe if it's a copy
            activeGames.set(gameId, currentData);
        }
    } else {
    ...
    try {
        ...
        if (diceRollValue === null && !helperBotError) {
            ...
            const timeoutUpdateClient = await pool.connect(); // Use a new client for this one-off update
            try {
        ...
        if (typeof diceRollValue !== 'number' /* || diceRollValue < 1 || diceRollValue > 64 */) { // Assuming slot values are 1-64
             // The original code didn't have a specific range check here, but good to note.
             // If helper returns something unexpected, it might break SLOT_PAYOUTS lookup.
             // For now, maintaining existing logic structure.
    ...
    // Delete the "Reels in Motion" message
    const spinningMessageId = gameData.gameMessageId; // gameMessageId was set to sentSpinningMsg.message_id
    if (spinningMessageId && bot) {
        await bot.deleteMessage(chatId, Number(spinningMessageId)).catch(e => {
            // console.warn(`${LOG_PREFIX_SLOT_START} Non-critical: Could not delete spinning message ID ${spinningMessageId}: ${e.message}`);
        });
        // Re-fetch gameData before modifying as async operations occurred
        const currentData = activeGames.get(gameId);
        if(currentData) {
            currentData.gameMessageId = null; // Clear it as it's deleted
            activeGames.set(gameId, currentData);
        }
    }
    ...
    // Re-fetch gameData from activeGames before modifying as async operations occurred
    const finalGameData = activeGames.get(gameId);
    if(!finalGameData){
        console.error(`${LOG_PREFIX_SLOT_START} GameData for ${gameId} disappeared before finalization. This should not happen.`);
        return;
    }
...
    // console.log(`${logPrefix} Starting grid generation.`); // Reduced logging
...
        }
    }
    // console.log(`${logPrefix} ${minesPlaced} mines placed.`); // Reduced logging
...
// DIAGNOSTIC VERSION: formatAndGenerateMinesMessageComponents is ASYNC, but the problematic 'await' in button text is temporarily removed.
...
// updateMinesGameMessage remains the same as last provided version, calling the above
...
// handleMinesDifficultySelectionCallback (ensure it uses corrected formatAndGenerateMinesMessageComponents via updateMinesGameMessage)
...
async function handleStartMinesCommand(msg, args, userObj) { // userObj is passed directly by the command router
    const userId = String(userObj.telegram_id);
    const chatId = String(msg.chat.id);
    const chatType = msg.chat.type;
    const LOG_PREFIX_MINES_START = `[Mines_StartOffer_HTML_UserCap_TypeLock_V2 UID:${userId} CH:${chatId}]`; // Updated log
    ...
    // userObj is already fetched and passed in
    if (BigInt(userObj.balance) < betAmountLamports) {
    ...
    // --- MODIFIED: Per-Group, Per-Game-Type Lock Check for MINES_OFFER ---
    const gameSession = await getGroupSession(chatId, msg.chat.title || `Group Chat ${chatId}`);
    const gameTypeToLock = GAME_IDS.MINES_OFFER;

    if (gameSession.activeGamesByTypeInGroup.has(gameTypeToLock)) {
        const existingGameIdInGroup = gameSession.activeGamesByTypeInGroup.get(gameTypeToLock);
        const existingGameInstance = activeGames.get(existingGameIdInGroup);

        const nonTerminalOfferStates = ['pending_offer', 'pending_unified_offer', 'awaiting_difficulty'];
        if (existingGameInstance && (nonTerminalOfferStates.includes(existingGameInstance.status) || (existingGameInstance.status && !existingGameInstance.status.startsWith('game_over_')))) {
            await safeSendMessage(chatId, `⏳ Hold your shovels, ${playerRefHTML}! A Mines game offer (Type: <code>${escapeHTML(existingGameInstance.type.replace(/_/g, " "))}</code>) is already active in this group. Please wait.`, { parse_mode: 'HTML' });
            return;
        } else if (!existingGameInstance) {
            console.warn(`${LOG_PREFIX_MINES_START} Stale game ID ${existingGameIdInGroup} found in group session for type ${gameTypeToLock}. Clearing.`);
            await updateGroupGameDetails(chatId, null, gameTypeToLock, null); // Clear this specific type lock
        }
    }
    // --- END MODIFIED Lock Check ---
    ...
    activeGames.set(offerId, offerData);
    // This Mines offer WILL lock the group session for MINES_OFFER type
    await updateGroupGameDetails(chatId, offerId, GAME_IDS.MINES_OFFER, betAmountLamports);
    console.log(`${LOG_PREFIX_MINES_START} Mines offer ${offerId} created and stored (group lock for MINES_OFFER set).`);

    let difficultyButtons = [];
    for (const diffKey in MINES_DIFFICULTY_CONFIG) {
        const diffConfig = MINES_DIFFICULTY_CONFIG[diffKey];
        difficultyButtons.push({
            text: `${diffConfig.emoji || TILE_EMOJI_MINE} ${escapeHTML(diffConfig.label)}`,
    ...
    if (sentMessage?.message_id) {
        const currentOffer = activeGames.get(offerId); // Re-fetch
        if (currentOffer) {
            currentOffer.offerMessageId = String(sentMessage.message_id);
            activeGames.set(offerId, currentOffer); // Save message ID
        } else {
            // Offer was somehow removed, clean up message and lock
             if(bot && sentMessage.message_id) await bot.deleteMessage(chatId, sentMessage.message_id).catch(()=>{});
             await updateGroupGameDetails(chatId, null, GAME_IDS.MINES_OFFER, null); // Clear lock
             return;
        }
        ...
        setTimeout(async () => {
            ...
            if (timedOutOffer && timedOutOffer.status === 'awaiting_difficulty') {
                ...
                activeGames.delete(offerId);
                await updateGroupGameDetails(chatId, null, GAME_IDS.MINES_OFFER, null); // Clear specific group lock
...
            break;
        case GAME_IDS.COINFLIP_PVP:
            FAMILY_LOCK_KEY_FOR_THIS_GAME = GAME_IDS.COINFLIP_UNIFIED_OFFER; // Assigning family lock key
            gameDisplayNameForMessages = "Coinflip";
            break;
        case GAME_IDS.RPS_PVP:
            FAMILY_LOCK_KEY_FOR_THIS_GAME = GAME_IDS.RPS_UNIFIED_OFFER; // Assigning family lock key
...
                const ledgerGameIdRef = offerData.gameId || offerId;
...
                activeGames.delete(offerId); // Delete the DIRECT_PVP_CHALLENGE offer object
...
                }
                        ...
                        // *** MODIFIED SECTION FOR CANONICAL FAMILY ID LOCK ***
                        // Call the specific game starter function. The starter function will now be responsible
                        // for creating the active game instance and updating the group lock with the FAMILY_LOCK_KEY.
                        // However, for consistency with how unified offers are handled, we can update the lock here
                        // immediately after creating the game object and *before* calling the game starter.
...
                    activeGames.set(pvpGameIdD21, pvpGameDataD21);
                        if (FAMILY_LOCK_KEY_FOR_THIS_GAME) { // Which is GAME_IDS.DICE_21_UNIFIED_OFFER
                           await updateGroupGameDetails(offerData.originalGroupId, pvpGameIdD21, FAMILY_LOCK_KEY_FOR_THIS_GAME, offerData.betAmount);
                        }
                    await startDice21PvPInitialDeal(pvpGameIdD21);
                } else if (offerData.gameToStart === GAME_IDS.DICE_ESCALATOR_PVP) {
                        // startDiceEscalatorPvPGame_New will handle the updateGroupGameDetails itself with the correct family key
...
                    // startDuelPvPGameSequence is called after activeGames.set and updateGroupGameDetails
                        const pvpGameIdDuel = generateGameId(GAME_IDS.DUEL_PVP);
                        const pvpGameDataDuel = { /* ... setup ... */ gameId: pvpGameIdDuel, type: GAME_IDS.DUEL_PVP, initiator:{userObj:freshInitiator, userId:freshInitiator.telegram_id, mention:getPlayerDisplayReference(freshInitiator)}, opponent:{userObj:freshTarget, userId:freshTarget.telegram_id, mention:getPlayerDisplayReference(freshTarget)}, chatId: offerData.originalGroupId, chatType: originalChatTypeFromGroup, betAmount: offerData.betAmount /* other fields */};
                        activeGames.set(pvpGameIdDuel, pvpGameDataDuel);
                        if (FAMILY_LOCK_KEY_FOR_THIS_GAME) { // Which is GAME_IDS.DUEL_UNIFIED_OFFER
                            await updateGroupGameDetails(offerData.originalGroupId, pvpGameIdDuel, FAMILY_LOCK_KEY_FOR_THIS_GAME, offerData.betAmount);
                        }
                    await startDuelPvPGameSequence(pvpGameIdDuel);
                } else if (offerData.gameToStart === GAME_IDS.COINFLIP_PVP) {
                        // startCoinflipPvPGame will handle its own family lock update
...
                    await startCoinflipPvPGame(coinflipInitiatorData, freshTarget, null, true /* betsAlreadyDeducted */);
                } else if (offerData.gameToStart === GAME_IDS.RPS_PVP) {
                        // startRPSPvPGame will handle its own family lock update
...
                    await startRPSPvPGame(rpsInitiatorData, freshTarget, null, true /* betsAlreadyDeducted */);
                } else {
                    console.error(`${logPrefix} Unknown gameToStart for direct challenge: ${offerData.gameToStart}`);
                    if (FAMILY_LOCK_KEY_FOR_THIS_GAME) { // If a family lock was associated with this original offer
                        await updateGroupGameDetails(originalChatIdFromGroup, null, FAMILY_LOCK_KEY_FOR_THIS_GAME, null);
                    }
                }
                        // *** END OF MODIFIED SECTION FOR CANONICAL FAMILY ID LOCK ***
